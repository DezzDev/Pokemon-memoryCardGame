...
function App() {
  ...
  const fetchData = async (pkmsCount: number) => {
    try {
      ...
      // function to generate a ramndom number in specific range, and how many we want
  ...
  const shuffleCards = (data: Pokemon[]) => {
    ...
    // create a deck with less data of pokemons
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const pkmsCount = formData.get("pkmsCount");

    if (!pkmsCount) {
      console.error("No se encontro la entrada en el formData");
    } else {
      // convert value to number
      const n = Number(pkmsCount);
      // if is not at number
      if (isNaN(n)) {
        console.error("No se pudo convertir el valor a númerico");
      } else {
